...
function relax_system()
    ...
    relax(sim; max_steps=5000, stopping_dmdt=1e-5, using_time_factor=false)
...
function drive_skyrmion(; alpha=0.04, omega=10.0, dt=20.0, steps=1000, delta=pi,
                        direction=110, basename="empty")
    ...
    return npzwrite(basename * ".npy", Array(sim.spin))
end
...
function plot_XY(filename)
    ...
    ax = Axis(fig[1, 1]; xlabel="Time", ylabel="Displacement (a)")
...
direction = 110;
delta = 0
basename = @sprintf("assets/pos_alpha_%g_omega_%g_delta_%g_direction_%03d", alpha, omega,
                    delta / pi, direction)
if !isfile(basename * ".txt")
    drive_skyrmion(; alpha=alpha, omega=omega, delta=delta, direction=direction,
                   basename=basename)
end
