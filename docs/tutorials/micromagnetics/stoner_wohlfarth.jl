...
# date: 2022-10-03
# description: an example to demostrate the Stoner–Wohlfarth model using NuMag
...
```
where $t=\tan ^{1 / 3} \theta$. Specifically, $h_s=1/2$ if $\theta=\pi/4$. In this example, we use NuMag to demostrate
this result.
...
we have ignored the demagnetization field. The NuMag script is shown below:
======================#
using NuMag
