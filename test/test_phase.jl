...
function phase_fft()
    ...
    Ms = 1e5
    phi = JuMag.compute_magnetic_phase_fft(mx_m, my_m, dx, dy, Lz, Ms, Nx=Nx, Ny=Ny)
    return phi
end
...
function phase_direct()
    ...
    phi = zeros(Nx, Nx)
    for i = 1:Nx, j=1:Ny
        phi[i, j] = JuMag.compute_magnetic_phase_direct(mx_m, my_m, dx, dy, Lz, Ms, i-80+16, j-80+32)
    end
    return phi
end
