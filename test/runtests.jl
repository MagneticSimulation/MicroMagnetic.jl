using MicroMagnetic
using Test


include("test_mesh.jl")
include("test_util.jl")
include("test_fill.jl")
include("test_init_m0.jl")
include("test_shapes.jl")
include("test_zeeman.jl")
include("test_anis.jl")
include("test_exch.jl")
include("test_stencil_pbc.jl")
include("test_magnetoelastic.jl")
include("test_dmi.jl")
include("test_demag.jl")
include("test_demag_packed.jl")
include("test_demag_pbc3d.jl")
include("test_demag_pbc2d.jl")
include("test_demag_pbc1d.jl")
include("test_demag_pbc_mesh.jl")
include("test_rm_charges.jl")
include("test_fields.jl")
include("test_sim.jl")
include("test_sim_with.jl")
include("test_set_driver.jl")
include("test_global_state.jl")
include("test_llg.jl")
include("test_illg.jl")
include("test_sllg.jl")
include("test_spatial_llg.jl")
include("test_gpsm.jl")
include("test_fill_equivalence.jl")
include("test_relax.jl")
include("test_pins.jl")
include("test_stt.jl")
include("test_slonczewski.jl")
include("test_sot.jl")
include("test_torque.jl")
include("test_sahe_torque.jl")
include("test_ovf.jl")
include("test_interlayer.jl")
include("test_skyrmion_number.jl")
include("test_phase.jl")
include("test_hysteresis.jl")

include("atomistic/test_mesh.jl")
include("atomistic/test_sim_tube.jl")
include("atomistic/test_llg.jl")
include("atomistic/test_fields.jl")
include("atomistic/test_exch.jl")
include("atomistic/test_dmi.jl")
include("atomistic/test_anis.jl")

include("test_mc.jl")
include("test_neb.jl")
include("test_voronoi.jl")
include("test_ltem.jl")
include("transition/test_transition.jl")
include("transition/test_solvers.jl")

include("eigen/test_cubic.jl")
include("eigen/test_afm.jl")
include("eigen/test_demag.jl")
include("eigen/test_build_matrix_types.jl")

include("fem/test_mesh.jl")
include("fem/test_interpolate.jl")
include("fem/test_llg.jl")
include("fem/test_fields.jl")
include("fem/test_demag.jl")

include("adjoint/test_duality.jl")
include("adjoint/test_steady.jl")
include("adjoint/test_sweepmatch.jl")
