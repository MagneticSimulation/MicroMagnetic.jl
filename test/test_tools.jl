...
function test_projection()
    ...
    m_padded = MicroMagnetic.vector_padding(m, 128,128,128)
    m = MicroMagnetic.vector_field_projection(m_padded, 0.0, "x")
...
function test_projection2()
    m = zeros(3,5,5,5)
    m[3,:,:,:] .= 1
    m_padded = MicroMagnetic.vector_padding(m, 9,9,9)
    mp = MicroMagnetic.vector_field_projection(m_padded, 0.0, "x")
    ...
    mp = MicroMagnetic.vector_field_projection(m_padded, 90.0, "x")
    mx, my, mz = mp[1,:,:], mp[2,:,:], mp[3,:,:]
    @test abs(mx[3,3] - 0)< 1e-6
    @test abs(mx[2,2] - 0) < 1e-6
    ...
    mp = MicroMagnetic.vector_field_projection(m_padded, 90.0, "y")
    mx, my, mz = mp[1,:,:], mp[2,:,:], mp[3,:,:]
