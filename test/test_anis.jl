...
function test_anis()
    ...
    @test isapprox(field[2], 2 * Ku / (MicroMag.mu_0 * Ms) * dot(m, axis) * axis[2])
    @test isapprox(field[3], 2 * Ku / (MicroMag.mu_0 * Ms) * dot(m, axis) * axis[3])
...
function test_cubic_anis()
    ...
    @test isapprox(field[1], 1 / (MicroMag.mu_0 * Ms) * 4 * Kc * 0.6^3)
    @test isapprox(field[2], 1 / (MicroMag.mu_0 * Ms) * 4 * Kc * 0.8^3)
    @test isapprox(field[3], 1 / (MicroMag.mu_0 * Ms) * 4 * Kc * 0^3)
