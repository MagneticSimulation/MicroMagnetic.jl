#Tests for the true 1D-periodic demag (add_demag(...; pbc1d=true) with exactly
#one of Nx/Ny/Nz > 0, see src/micro/demag_pbc1d.jl).  References:
#  * brute force: explicit image sum |p| <= P along the periodic axis
#    (the other two directions open), assembled independently of the FFT pipeline
#  * macro PBC (the same axis, N images, no tail) converges to pbc1d
#  * uniform magnetization along the periodic axis: the response is the discrete
#    lattice (Lorentz-type) self field carried by the periodic-DC column; it is
#    built from the direct long axial sum, so it must not depend on Ic
...
#explicit image sum |p| <= P along one axis of the Newell tensor at signed
#displacements; the field convention matches DirectDemag (h = -sum N * mu0_Ms/mu_0 * m)
function pbc1d_bruteforce_field(sim; axis=1, P=6)
    mesh = sim.mesh
...
function pbc1d_build(mesh, m; axis=1, Ic=2, macroN=0)
    sim = Sim(mesh)
    set_Ms(sim, 8e5)
    init_m0(sim, m; norm=false)
    Ns = macroN > 0 ? (axis == 1 ? (macroN, 0, 0) : axis == 2 ? (0, macroN, 0) : (0, 0, macroN)) :
         (axis == 1 ? (Ic, 0, 0) : axis == 2 ? (0, Ic, 0) : (0, 0, Ic))
    if macroN > 0
        add_demag(sim; Nx=Ns[1], Ny=Ns[2], Nz=Ns[3])
    else
        add_demag(sim; pbc1d=true, Nx=Ns[1], Ny=Ns[2], Nz=Ns[3])
    end
...
function test_pbc1d_uniform()
    ...
    for axis in (1, 2, 3)
        ...
        h2 = pbc1d_build(mesh, m; axis=axis, Ic=2)
        h6 = pbc1d_build(mesh, m; axis=axis, Ic=6)
        Tf = eltype(h6)
        tol = Tf == Float32 ? 1e-4 : 1e-5
        @test maximum(abs, h6) <= tol * 8e5          #converged: H ~ 0
        @test maximum(abs, h2) <= 1e-3 * 8e5         #the default Ic=2 already small
    end
end
...
function test_pbc1d_bruteforce()
    ...
    for axis in (1, 2, 3)
        ...
        h = pbc1d_build(mesh, m; axis=axis, Ic=2)
        sim = Sim(mesh)
        set_Ms(sim, 8e5)
        init_m0(sim, m; norm=false)
        h_bf = pbc1d_bruteforce_field(sim; axis=axis, P=4)
...
function test_pbc1d_macro_convergence()
    ...
    #tight pbc1d reference (Ic=6) so the macro image truncation dominates
    h_pbc = pbc1d_build(mesh, m; axis=1, Ic=6)
    scale = maximum(abs, h_pbc)
    e = [maximum(abs, pbc1d_build(mesh, m; axis=1, macroN=N) - h_pbc) for N in (2, 4, 8)]
