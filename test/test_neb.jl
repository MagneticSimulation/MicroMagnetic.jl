...
function test_convert()
    ...
    m0 = [r, theta, psi]
    cartesian = MicroMag.spherical2cartesian(m0, 1)
    ...
    spherical = MicroMag.cartesian2spherical(cartesian, 1)
...
function test_inner_product()
    x1 = [1, 2., 3.]
    x2 = [4., 5., 6.]
    z = MicroMag.inner_product(x1,x2, 1)
    @test isapprox(z[1], 4+10+18)
end

function test_slerp()
    t1 = pi/2
    x1 = MicroMag.slerp([1, 0, 0.], [cos(t1), sin(t1), 0.], 1/3, 1)
    @test isapprox(x1, [cos(t1/3), sin(t1/3), 0.])

    t2 = 5/4 * pi
    t2_c = t2 - 2*pi
    x2 = MicroMag.slerp([1, 0, 0.], [cos(t2), sin(t2), 0.], 2/3, 1)
...
function test_init_images()
    ...
    x2 = [cos(t1), sin(t1), 0.]
    x3 = [-1, 0., 0.]
    images = MicroMag.init_images((x1,x2, x3), (2,1), 1)
...
function test_init_neb()
    ...
    init_m0(sim, (0,1,0.))
    m1 = Array(sim.spin)
    neb = MicroMag.NEB(sim, (m0, m1), (1,); name="test")
    println(neb.images)
end
test_init_neb()
