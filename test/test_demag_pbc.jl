...
function test_field_macro(;gpu=false)
    ...
    f1 = compute_field_macro(mesh1, 500, 0, 0, gpu=gpu)  #1d case
    println(f1)
    @test isapprox(f1[1], 0, atol=1e-6)
    @test isapprox(f1[2], -0.5, atol=1e-6)
    @test isapprox(f1[3], -0.5, atol=1e-6)

    f2 = compute_field_macro(mesh3, 5, 5, 0, gpu=gpu)  #2d case
    ...
    f3 = compute_field_macro(mesh1, 31, 31, 0, gpu=gpu) #self-consistency
    f4 = compute_field_macro(mesh2, 10, 10, 0, gpu=gpu)
