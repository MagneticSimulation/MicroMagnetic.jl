...
function plot_m()
    ...
    ax = Axis(fig[1, 1]; xlabel="Time step (s)", ylabel="Absolute error", xscale=log10,
              yscale=log10)

    scatterlines!(ax, error[1, :], error[2, :]; markersize=12, marker=:rect, color=:sienna1,
                  label="RungeKutta")
    scatterlines!(ax, error[1, :], error[3, :]; markersize=12, color=:slateblue1,
                  strokecolor=:transparent, label="RungeKuttaCayley")

    axislegend(; position=(0.95, 0.05), labelsize=16)

    return save("error.pdf", fig)
end
