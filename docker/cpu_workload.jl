...
# Mirrors the hot CPU paths from src/precompile.jl so they are baked into
# the sysimage at build time (PrecompileTools workloads do not run during
# create_sysimage).

# atomistic
mesh = MicroMagnetic.CubicMesh(nx=16, ny=16, nz=16, dx=1e-9, dy=1e-9, dz=1e-9, pbc="xyz")
sim = MicroMagnetic.Sim(mesh; driver="SD", name="_sysimage_wl")
