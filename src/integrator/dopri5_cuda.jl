...
function DormandPrinceGPU(n_total::Int64, rhs_fun, tol::Float64)
    Float = _cuda_using_double.x ? Float64 : Float32
    errors = CUDA.zeros(Float,3*n_total)
    k1 = CUDA.zeros(Float,3*n_total)
    k2 = CUDA.zeros(Float,3*n_total)
    k3 = CUDA.zeros(Float,3*n_total)
    k4 = CUDA.zeros(Float,3*n_total)
    k5 = CUDA.zeros(Float,3*n_total)
    k6 = CUDA.zeros(Float,3*n_total)
    k7 = CUDA.zeros(Float,3*n_total)
    facmax = 5.0
    facmin = 0.2
    safety = 0.824
...
function dopri5_step_inner_GPU(sim::AbstractSim, step::Float64, t::Float64)
  ...
  @cuda blocks=blk threads=thr dopri5__step6__kernel!(sim.spin,
                               ...
                               step, N)
  normalise(sim.spin, sim.n_total) #if we want to copy k7 to k1, we should normalise it here.
