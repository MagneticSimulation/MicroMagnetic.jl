...
function create_driver_gpu(driver::String, integrator::String, n_nodes::Int64)
    ...
    if driver == "SD"
        gk = CUDA.zeros(Float,3*n_nodes)
        field = CUDA.zeros(Float,3*n_nodes)
        ss = CUDA.zeros(Float, n_nodes)
        sf = CUDA.zeros(Float, n_nodes)
        ff = CUDA.zeros(Float, n_nodes)
        max_tau = 1.0
    ...
    if driver=="LLG"
        ...
        field = CUDA.zeros(Float, 3*n_nodes)
        if integrator == "Heun"
            dopri5 = ModifiedEulerGPU(n_nodes, llg_call_back_gpu, 1e-14)
        elseif integrator == "RungeKutta"
            dopri5 = RungeKuttaGPU(n_nodes, llg_call_back_gpu, 5e-14)
        elseif integrator == "DormandPrince"
            dopri5 = DormandPrinceGPU(n_nodes, llg_call_back_gpu, tol)
        else
            dopri5 = DormandPrinceCayleyGPU(n_nodes, llg_cayley_call_back_gpu, tol)
        end
        return LLG_GPU(true, Float(0.1), Float(2.21e5), dopri5, tol, field)
    elseif driver=="LLG_STT"
        tol = 1e-6
        field = CUDA.zeros(Float, 3*n_nodes)
        ux = CUDA.zeros(Float, n_nodes)
        uy = CUDA.zeros(Float, n_nodes)
        uz = CUDA.zeros(Float, n_nodes)
        hstt = CUDA.zeros(Float, 3*n_nodes)
        fun = t::Float64 -> 1.0
        if integrator == "Heun"
            dopri5 = ModifiedEulerGPU(n_nodes, llg_stt_call_back_gpu, 1e-14)
        elseif integrator == "RungeKutta"
            dopri5 = RungeKuttaGPU(n_nodes, llg_stt_call_back_gpu, 5e-14)
        elseif integrator == "DormandPrince"
            dopri5 = DormandPrinceGPU(n_nodes, llg_stt_call_back_gpu, tol)
        else
            dopri5 = DormandPrinceCayleyGPU(n_nodes, llg_stt_cayley_call_back_gpu, tol)
        end
        return LLG_STT_GPU(Float(0.5), Float(0), Float(2.21e5), dopri5, tol, ux, uy, uz, hstt, field, fun)
    elseif driver=="LLG_CPP"
        tol = 1e-6
        field = CUDA.zeros(Float, 3*n_nodes)
        aj = CUDA.zeros(Float, n_nodes)
        fun = t::Float64 -> 1.0
        p = (0,0,1)
        if integrator == "Heun"
            dopri5 = ModifiedEulerGPU(n_nodes, llg_cpp_call_back_gpu, 1e-14)
        elseif integrator == "RungeKutta"
            dopri5 = RungeKuttaGPU(n_nodes, llg_cpp_call_back_gpu, 5e-14)
        elseif integrator == "DormandPrince"
            dopri5 = DormandPrinceGPU(n_nodes, llg_cpp_call_back_gpu, tol)
        else
    ...
    elseif driver=="LLG_STT_CPP"
        ...
        p = (0,0,1)
        if integrator == "Heun"
            dopri5 = ModifiedEuler(n_nodes, llg_stt_call_back_gpu, 1e-14)
        elseif integrator == "RungeKutta"
            dopri5 = RungeKuttaGPU(n_nodes, llg_stt_call_back_gpu, 5e-14)
        elseif integrator == "DormandPrince"
            dopri5 = DormandPrinceGPU(n_nodes, llg_stt_cpp_call_back_gpu, tol)
        else
            dopri5 = DormandPrinceCayleyGPU(n_nodes, llg_stt_cpp_cayley_call_back_gpu, tol)
        end
