...
function init_demag_gpu_II(sim::MicroSim, Nx::Int, Ny::Int, Nz::Int)
  ...
  field = zeros(Float, 3*sim.n_total)
  energy = zeros(Float, sim.n_total)
  spin = CUDA.zeros(Float, 3*sim.n_total)
...
function effective_field(demag::DemagGPU, sim::MicroSimGPU, spin::CuArray{T, 1}, t::Float64) where {T<:AbstractFloat}
  ...
  @cuda blocks=blocks_n threads=threads_n compute_energy_kernel!(sim.energy, spin, sim.field, sim.Ms, mesh.volume, sim.n_total)
  return nothing
end
