function spherical2cartesian(spherical, n_total)
    ...
    cartesian = KernelAbstractions.zeros(dev, eltype(spherical), 3*n_total)
...
function cartesian2spherical(cartesian, n_total)
    ...
    spherical = KernelAbstractions.zeros(dev, eltype(cartesian), 3*n_total)
...
function inner_product(x1, x2, n_total)
    dev = get_backend(x1)
    res = KernelAbstractions.zeros(dev, eltype(x1), n_total)
...
function slerp(x1, x2, t, n_total::Int)
    dev = get_backend(x1)
    res = KernelAbstractions.zeros(dev, eltype(x1), 3*n_total)
