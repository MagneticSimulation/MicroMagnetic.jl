...
function init_m0_skyrmion_lattice(sim::MicroMag.AbstractSim, lambda::Float64; p=-1, c="l", type="B")
    mesh = sim.mesh
    nx,ny,nz = mesh.nx,mesh.ny,mesh.nz
    dx,dy,dz = mesh.dx,mesh.dy,mesh.dz
    ...
    for i=1:nx, j=1:ny
        ...
        if c=="l"
            mx,my = MicroMag.rotation_2d(180., [mx, my])
        end

        if type == "B"
          mx , my = MicroMag.rotation_2d(90., [mx, my])
          b[1,i,j,:] .= mx
          b[2,i,j,:] .= my
